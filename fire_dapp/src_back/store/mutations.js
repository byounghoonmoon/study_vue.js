...
export default {
    ...
    [Constant.CHANGE_CENTER_MODE] : (state,payload) => {
     ...
     // 센터 추가
     [Constant.ADD_CENTER] : (state,payload) => {
        state.initCenterInfoList.push({"centerCd":payload.centerCd,"centerNm":payload.centerNm});
    } ,
     // 보험사 추가  
     [Constant.ADD_INSURER] : (state,payload) => {
        state.initInsurerInfoList.push({"insCd":payload.insCd,"insNm":payload.insNm});
    } 
